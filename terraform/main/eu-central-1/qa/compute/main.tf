...
module "eks" {
  ...
  create_cloudwatch_log_group            = lookup(var.eks, "create_cloudwatch_log_group", false)
  cloudwatch_log_group_retention_in_days = lookup(var.eks, "cloudwatch_log_group_retention_in_days", 7)
  cluster_enabled_log_types              = lookup(var.eks, "cluster_enabled_log_types", [])
  cluster_endpoint_private_access        = lookup(var.eks, "cluster_endpoint_private_access", true)
  cluster_endpoint_public_access         = lookup(var.eks, "cluster_endpoint_public_access", false)
